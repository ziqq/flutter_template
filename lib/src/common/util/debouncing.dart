...
final class Debouncing<T> extends Stream<DebouncingStatus> implements Sink<T Function()> {
  ///  Debouncing
  ///  Have method [debounce]
  /// Must be closed with [close] method
  Debouncing({Duration duration = const Duration(milliseconds: 500)})
  ...
  // ignore: close_sinks
  final StreamController<DebouncingStatus> _stateSC = StreamController<DebouncingStatus>.broadcast(sync: true);
  ...
  Future<T?> debounce(T Function() fn) async {
  ...
  StreamSubscription<DebouncingStatus> listen(
    ...
    bool? cancelOnError,
  }) => _stateSC.stream.listen(onData, onError: onError, onDone: onDone, cancelOnError: cancelOnError);
