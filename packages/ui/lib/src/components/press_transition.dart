...
/// Defines the type of press transition to apply.
enum PressTransitionType {
  /// A press transition that scales the child down slightly.
  down(.97),

  /// A press transition that scales the child up slightly.
  up(1.07);

  /// Creates a press transition type with the given scale factor.
  const PressTransitionType(this.scale);

  /// The scale factor to apply when the press is fully engaged.
  final double scale;
}

/// Builds the widget tree for a [PressTransition].
typedef PressTransitionBuilder = Widget Function(BuildContext context, double pressProgress, Widget? child);
...
class _PressTransitionState extends State<PressTransition> with SingleTickerProviderStateMixin {
  ...
  @override
  Widget build(BuildContext context) => NotificationListener<_PressTransitionNotification>(
    ...
    child: Listener(
      ...
      child: GestureDetector(
        ...
        child: AnimatedBuilder(
          ...
          child: widget.child,
          builder: (context, child) => widget.builder(context, widget.curve.transform(_controller.value), child),
        ),
      ),
    ),
  );
